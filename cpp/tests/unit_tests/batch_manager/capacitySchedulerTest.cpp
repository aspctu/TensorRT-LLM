...
TEST_F(CapacitySchedulerTest, SchedulerCreditAgesRequestsWithinTierWithoutKvCache)
{
    // With hasKvCacheManager=false the capacity scheduler falls back to the
    // request-count-only path. Fairness still applies through the shared
    // active-request ordering layer.
    auto capacityScheduler = CapacityScheduler(1, CapacitySchedulerPolicy::kGUARANTEED_NO_EVICT, false);

    auto firstReq = createRequest(10, 20, 0, std::nullopt, 0.5);
    auto secondReq = createRequest(10, 20, 1, std::nullopt, 0.5);
    firstReq->setSchedulerControlsEnabled(true);
    secondReq->setSchedulerControlsEnabled(true);
    firstReq->setSchedulerOrganizationHash(1);
    secondReq->setSchedulerOrganizationHash(1);
    RequestList activeRequests{firstReq, secondReq};

    auto [firstScheduled, firstDisagg, firstPaused] = capacityScheduler(activeRequests);
    ASSERT_TRUE(firstDisagg.empty());
    ASSERT_TRUE(firstPaused.empty());
    ASSERT_EQ(firstScheduled.size(), 1);
    EXPECT_EQ(firstScheduled.front()->mRequestId, firstReq->mRequestId);

    auto [secondScheduled, secondDisagg, secondPaused] = capacityScheduler(activeRequests);
    ASSERT_TRUE(secondDisagg.empty());
    ASSERT_TRUE(secondPaused.empty());
    ASSERT_EQ(secondScheduled.size(), 1);
    EXPECT_EQ(secondScheduled.front()->mRequestId, secondReq->mRequestId);
    EXPECT_GT(firstReq->getSchedulerCredit(), 0.0);
}

TEST_F(CapacitySchedulerTest, SchedulerBalancesOrganizationsWithinPriorityTierWithoutKvCache)
{
    auto capacityScheduler = CapacityScheduler(1, CapacitySchedulerPolicy::kGUARANTEED_NO_EVICT, false);

    auto orgAReq = createRequest(64, 20, 0, std::nullopt, 0.5);
    auto orgBReq = createRequest(16, 20, 1, std::nullopt, 0.5);
    orgAReq->setSchedulerControlsEnabled(true);
    orgBReq->setSchedulerControlsEnabled(true);
    orgAReq->setSchedulerOrganizationHash(1);
    orgBReq->setSchedulerOrganizationHash(2);

    RequestList activeRequests{orgAReq, orgBReq};
    auto [firstScheduled, firstDisagg, firstPaused] = capacityScheduler(activeRequests);
    ASSERT_TRUE(firstDisagg.empty());
    ASSERT_TRUE(firstPaused.empty());
    ASSERT_EQ(firstScheduled.size(), 1);
    EXPECT_EQ(firstScheduled.front()->mRequestId, orgBReq->mRequestId);

    bool sawHeavyOrgScheduled = false;
    for (int iteration = 0; iteration < 8; ++iteration)
    {
        auto [scheduledRequests, disaggRequests, pausedRequests] = capacityScheduler(activeRequests);
        ASSERT_TRUE(disaggRequests.empty());
        ASSERT_TRUE(pausedRequests.empty());
        ASSERT_EQ(scheduledRequests.size(), 1);
        if (scheduledRequests.front()->mRequestId == orgAReq->mRequestId)
        {
            sawHeavyOrgScheduled = true;
            break;
        }
    }
    EXPECT_TRUE(sawHeavyOrgScheduled);
}

TEST_F(CapacitySchedulerTest, SchedulerInterleavesOrganizationsWithinPriorityTierWithoutKvCache)
{
    auto capacityScheduler = CapacityScheduler(2, CapacitySchedulerPolicy::kGUARANTEED_NO_EVICT, false);

    auto orgAReq0 = createRequest(16, 20, 0, std::nullopt, 1.5);
    auto orgAReq1 = createRequest(16, 20, 1, std::nullopt, 1.5);
    auto orgBReq0 = createRequest(16, 20, 2, std::nullopt, 1.5);
    orgAReq0->setSchedulerControlsEnabled(true);
    orgAReq1->setSchedulerControlsEnabled(true);
    orgBReq0->setSchedulerControlsEnabled(true);
    orgAReq0->setSchedulerOrganizationHash(1);
    orgAReq1->setSchedulerOrganizationHash(1);
    orgBReq0->setSchedulerOrganizationHash(2);

    RequestList activeRequests{orgAReq0, orgAReq1, orgBReq0};
    auto [scheduledRequests, disaggRequests, pausedRequests] = capacityScheduler(activeRequests);

    ASSERT_TRUE(disaggRequests.empty());
    ASSERT_TRUE(pausedRequests.empty());
    ASSERT_EQ(scheduledRequests.size(), 2);
    EXPECT_EQ(scheduledRequests[0]->mRequestId, orgBReq0->mRequestId);
    EXPECT_EQ(scheduledRequests[1]->mRequestId, orgAReq0->mRequestId);
}

TEST_F(CapacitySchedulerTest, SchedulerPrefersLowerPressureOrganizationWithinPriorityTierWithoutKvCache)
{
    auto capacityScheduler = CapacityScheduler(2, CapacitySchedulerPolicy::kGUARANTEED_NO_EVICT, false);

    auto heavyOrgReq0 = createRequest(4096, 512, 0, std::nullopt, 1.5);
    auto heavyOrgReq1 = createRequest(4096, 512, 1, std::nullopt, 1.5);
    auto lightOrgReq = createRequest(128, 32, 2, std::nullopt, 1.5);
    heavyOrgReq0->setSchedulerControlsEnabled(true);
    heavyOrgReq1->setSchedulerControlsEnabled(true);
    lightOrgReq->setSchedulerControlsEnabled(true);
    heavyOrgReq0->setSchedulerOrganizationHash(1);
    heavyOrgReq1->setSchedulerOrganizationHash(1);
    lightOrgReq->setSchedulerOrganizationHash(2);

    RequestList activeRequests{heavyOrgReq0, heavyOrgReq1, lightOrgReq};
    auto [scheduledRequests, disaggRequests, pausedRequests] = capacityScheduler(activeRequests);

    ASSERT_TRUE(disaggRequests.empty());
    ASSERT_TRUE(pausedRequests.empty());
    ASSERT_EQ(scheduledRequests.size(), 2);
    EXPECT_EQ(scheduledRequests[0]->mRequestId, lightOrgReq->mRequestId);
    EXPECT_EQ(scheduledRequests[1]->mRequestId, heavyOrgReq0->mRequestId);
}

TEST_F(CapacitySchedulerTest, SchedulerPriorityTiersDominateOrganizationBalancingWithoutKvCache)
{
    auto capacityScheduler = CapacityScheduler(1, CapacitySchedulerPolicy::kGUARANTEED_NO_EVICT, false);

    auto highTierReq = createRequest(64, 20, 0, std::nullopt, 1.5);
    highTierReq->setSchedulerControlsEnabled(true);
    highTierReq->setSchedulerOrganizationHash(1);

    RequestList warmupRequests{highTierReq};
    for (int iteration = 0; iteration < 3; ++iteration)
    {
        auto [scheduledRequests, disaggRequests, pausedRequests] = capacityScheduler(warmupRequests);
        ASSERT_TRUE(disaggRequests.empty());
        ASSERT_TRUE(pausedRequests.empty());
        ASSERT_EQ(scheduledRequests.size(), 1);
        EXPECT_EQ(scheduledRequests.front()->mRequestId, highTierReq->mRequestId);
    }

    auto lowTierReq = createRequest(16, 20, 1, std::nullopt, 0.5);
    lowTierReq->setSchedulerControlsEnabled(true);
    lowTierReq->setSchedulerOrganizationHash(2);

    RequestList activeRequests{highTierReq, lowTierReq};
    auto [scheduledRequests, disaggRequests, pausedRequests] = capacityScheduler(activeRequests);
...
TEST_F(CapacitySchedulerTest, SchedulerPrioritizesHighestTierButUsesRemainingCapacityGuaranteedNoEvict)
{
    auto kvCacheManager = getKvCacheManager(4, 10, 400, 90);
    auto capacityScheduler
        = CapacityScheduler(4, CapacitySchedulerPolicy::kGUARANTEED_NO_EVICT, kvCacheManager != nullptr);

    auto highTierReqA = createRequest(32, 20, 0, std::nullopt, 2.5);
    auto highTierReqB = createRequest(24, 20, 1, std::nullopt, 2.5);
    auto lowTierReq = createRequest(16, 20, 2, std::nullopt, 0.5);
    highTierReqA->setSchedulerControlsEnabled(true);
    highTierReqB->setSchedulerControlsEnabled(true);
    lowTierReq->setSchedulerControlsEnabled(true);
    highTierReqA->setSchedulerOrganizationHash(1);
    highTierReqB->setSchedulerOrganizationHash(2);
    lowTierReq->setSchedulerOrganizationHash(3);

    RequestList activeRequests{highTierReqA, highTierReqB, lowTierReq};
    auto [scheduledRequests, disaggRequests, pausedRequests]
        = capacityScheduler(activeRequests, kvCacheManager, std::nullopt, std::nullopt);

    ASSERT_TRUE(disaggRequests.empty());
    ASSERT_TRUE(pausedRequests.empty());
    ASSERT_EQ(scheduledRequests.size(), 3);
    auto const highTierIds = std::vector<RequestIdType>{
        scheduledRequests[0]->mRequestId,
        scheduledRequests[1]->mRequestId,
    };
    EXPECT_THAT(highTierIds, ::testing::UnorderedElementsAre(highTierReqA->mRequestId, highTierReqB->mRequestId));
    EXPECT_EQ(scheduledRequests[2]->mRequestId, lowTierReq->mRequestId);
}

TEST_F(CapacitySchedulerTest, SchedulerPrioritizesHighestTierButUsesRemainingCapacityMaxUtilization)
{
    auto kvCacheManager = getKvCacheManager(4, 10, 400, 90);
    auto capacityScheduler
        = CapacityScheduler(4, CapacitySchedulerPolicy::kMAX_UTILIZATION, kvCacheManager != nullptr);
